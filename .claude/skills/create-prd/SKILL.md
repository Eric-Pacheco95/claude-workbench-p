...
## Examples
- /create-prd Build an autonomous task runner for your AI brain that executes safe tasks on a schedule
...
- Scan the input for multiple distinct actor types (e.g., "trader + Slack approver", "the user + autonomous dispatcher + monitoring agent", "admin + end user")
- If 2+ distinct actors are present AND the input does not already include user stories: generate user stories BEFORE writing requirements
  ...
  - Present stories to the user and confirm before proceeding — stories define WHAT to build; requirements define HOW it should behave
- If only 1 actor type (typical your AI brain task): skip this step silently
...
- **Model Annotation** — After the ISC Quality Gate passes, apply the keyword heuristic to each criterion and propose `| model: X |` annotations. Present them as a numbered review list and wait for the user to confirm, edit, or reject before writing to the PRD file:
  ...
  Present proposed annotations to the user:
  ...
  > ..."
  Wait for your response before writing. If your response is ambiguous or not clearly directed at the annotation list, ask once: "Confirming approval of the model routing list above before I write — correct?" before writing. If the user approves without changes, write all proposed annotations. If the user edits any annotation, use their version.
