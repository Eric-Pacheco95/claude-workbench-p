...
- /red-team --stride The your AI brain heartbeat system runs as a scheduled task with file-based state
...
- If input is too short to meaningfully red-team (fewer than 10 words, no plan or system described):
  - Print: "I need a plan, product, or system to stress-test. Provide a description or file path. Example: /red-team docs/jarvis/PRD.md"
  - STOP
...
- If `--thinking` and `--stride` are both present: run `--thinking` mode first, then append STRIDE analysis of your reasoning frameworks as a bonus section
...
- Promote the highest-severity finding pattern to the relevant agent or skill Critical Rules if it represents a recurring your AI brain-specific risk
