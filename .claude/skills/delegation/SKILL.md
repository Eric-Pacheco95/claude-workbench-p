...
- Classify the task by type:
  ...
  - **Parallel dispatch**: Multiple independent tasks with no dependencies → spawn N agents in parallel using `run_in_background: true`, report via scoreboard table as each completes. Prompt template per agent: identity ("You are your AI brain") + mission + context files to read + output format + destination file. Use when 3+ tasks are independent and research/design-scoped (not mutations)
  - **Research task**: Needs external information gathering → route to `/research`
  - **Manual task**: Requires your judgment, creativity, or personal input → flag for the user with context
...
- For each task, assess:
  ...
  - **Autonomy**: Can your AI brain handle alone, or does the user need to be involved?
...
| Unknown / novel | Flag for the user + suggest `/create-pattern` | — |
