...
- /visualize system "your AI brain architecture: hooks, MCP, skills, memory"
...
- After: Save to docs/{project}/ or orchestration/workflows/ (after approval)
...
- If type is `brain` → this feeds Phase 3D brain spec. Save output to `docs/brain_spec/` (create if needed).
- If type is `workflow` → save to `orchestration/workflows/` if the user approves.
- If type is `project` → save to `docs/{project}/` after approval.
...
- Write a signal to memory/learning/signals/{YYYY-MM-DD}_visualize-{slug}.md when a visualization reveals a structural insight about your AI brain architecture that was not obvious from the code or docs (e.g., missing link in a flow, unexpected coupling, critical path)
