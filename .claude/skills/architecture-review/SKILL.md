...
- /architecture-review docs/projects/my-project/PRD.md
...
- Present the framing to the user for confirmation before launching agents:
...
- Create a temp directory for agent outputs: `docs/_arch-review-{timestamp}/`
- Launch 3 Agent tool calls simultaneously in a single message (this is critical — they must run in parallel, not sequentially):
  ...
  **Agent 1: First-Principles Decomposition**
  ...
  - **Agent MUST write its findings to `docs/_arch-review-{timestamp}/first-principles.md` before returning** — this is critical for surviving context compaction in long sessions
  ...
  **Agent 2: Logical Fallacy Detection**
  ...
  - **Agent MUST write its findings to `docs/_arch-review-{timestamp}/fallacy-detection.md` before returning**
  ...
  **Agent 3: Red-Team (+ STRIDE if --stride flag or auto-detected)**
  ...
  - **Agent MUST write its findings to `docs/_arch-review-{timestamp}/red-team.md` before returning**
  ...
  **Agent 4 (only if --thinking flag): Reasoning Blindspot Check**
  ...
  - **Agent MUST write its findings to `docs/_arch-review-{timestamp}/thinking.md` before returning**
...
- Read all agent outputs from `docs/_arch-review-{timestamp}/` — do NOT rely on agent return values alone, as these may be lost to context compaction in long sessions
...
- Clean up the temp directory: delete `docs/_arch-review-{timestamp}/` after synthesis is complete
...
- **scope-too-broad:** proposal covers multiple independent decisions
  - recover: ask the user to split into separate reviews; each decision gets its own /architecture-review
...
- Confirm the temp directory docs/_arch-review-{timestamp}/ was deleted after synthesis
