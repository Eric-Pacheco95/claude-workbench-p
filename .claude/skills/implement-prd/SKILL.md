...
## Examples
- /implement-prd docs/jarvis/PRD.md
- /implement-prd docs/jarvis/PRD.md --phase 2A
...
- If PRD file not found at given path:
  - Print: "PRD not found at {path}. Check the path. Available PRDs:" followed by list from docs/*/PRD.md
...
Wait for your response. If he annotates items, write the annotations to the PRD file before proceeding. If he confirms Opus for all, proceed with no subagent dispatch.
...
- If the command returns a WARNING on stderr and falls back to full scope: print the warning to the user and proceed with full scope — do not silently ignore the fallback
...
- **Mid-build commit checkpoint**: After every 3-4 completed ISC items, prompt the user to commit: "Checkpoint: {N} ISC items verified. Run /commit to save progress?" This creates recovery points against context compaction. Do not auto-commit — wait for confirmation. If the user declines, continue building
...
For each completed ISC item, generate a scaffold sentence and prompt the user to confirm or edit before proceeding:
...
- Wait for your response before writing COMPLETION STATUS
- If the user edits a sentence, use their version verbatim
- If the user approves without editing (e.g. "looks good"), use the scaffold version
- Record all sentences in the VERIFY RESULTS table under OWNERSHIP CHECK column
- Do not write COMPLETION STATUS until the user has responded — this is a non-bypassable gate
...
- **Final commit prompt**: Run `git status` — if there are uncommitted changes, prompt: "BUILD complete and verified. Ready to commit? Run /commit or I can stage and commit now." Do not auto-commit — wait for your confirmation. If the user declines, proceed to /learning-capture
...
- VERIFY RESULTS: (1) OWNERSHIP CHECK table (ISC Item | the user summary | scaffold-or-edited), (2) approach retrospective, (3) evidence table (ISC Item | Method | Result | Evidence Type | Source | Content)
...
- **prd-not-found:** supplied file path does not exist
  - recover: check the path; PRDs are typically in `docs/<project>/PRD.md`
